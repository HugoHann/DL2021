...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        probabilitites = np.exp(scores) / np.sum(np.exp(scores), axis = 1).reshape(-1,1)
        ...
        grad_O2 = grad_Z3.dot(W3.T)
        grad_O2[layer2 <= 0] = 0 #the RELU function.

        grads["b2"] = np.sum(grad_O2, axis = 0)
        grads["W2"] = layer1.T.dot(grad_O2)
        grads["W2"] += 2*reg * W2


        grad_O1 = grad_O2.dot(W2.T)
        grad_O1[layer1 <= 0] = 0 #the RELU function.
